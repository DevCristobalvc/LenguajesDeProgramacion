...
# cada colección está implementada como una lista de diccionarios:
publishers = []
videogames = []
users = []
reviews = []
...
def addPublisher(id, name, country, total_sales):
    newPublisher = {"id": id, "name": name, "country": country, "total_sales": float(total_sales)}
    publishers.append(newPublisher)

def filterPublisherByTotalSales_iter(totalSales):
    filteredList = []
    for rec in publishers:
        if rec["total_sales"] == float(totalSales):
...
def filterPublisherByTotalSales_FilterLambda(totalSales):
    ...
    filterFunction2 = lambda x: x["total_sales"] == float(totalSales)
